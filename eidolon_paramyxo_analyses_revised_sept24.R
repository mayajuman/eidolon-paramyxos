...
setwd("~/Documents/PhD/eidolon/paramyxo submission/round 2/round 3/round 4")
...
wilcox.test(MFI.data$`Analyte 35 GHV`, MFI.data$`Analyte 26 MOCK`, paired=TRUE)
wilcox.test(MFI.data$`Analyte 46 NIV`, MFI.data$`Analyte 26 MOCK`, paired=TRUE)
wilcox.test(MFI.data$`Analyte 43 HEV`, MFI.data$`Analyte 26 MOCK`, paired=TRUE)
wilcox.test(MFI.data$`Analyte 29 MOJV`, MFI.data$`Analyte 26 MOCK`, paired=TRUE)
wilcox.test(MFI.data$`Analyte 53 CEDV`, MFI.data$`Analyte 26 MOCK`, paired=TRUE)
wilcox.test(MFI.data$`Analyte 28 MENV`, MFI.data$`Analyte 26 MOCK`, paired=TRUE)
...
ghvbox <- MFI.mock.corrected.wide %>% drop_na(Sex) %>%
  ...
  geom_boxplot(aes(fill=Sex), width=50, outlier.shape = NA, varwidth = TRUE) +
...
ggarrange(ghvbox, nivbox, ncol=1, nrow=2, labels = c("A","B"))
#ggsave("Fig2.png", width = 150, height = 234, units= "mm", bg="white")

ggarrange(hevbox, mojvbox, cedvbox, ncol=2, nrow=2, labels = c("A","B","C"))
#ggsave("FigS2.png", width = 300, height = 300, units= "mm", bg="white")
